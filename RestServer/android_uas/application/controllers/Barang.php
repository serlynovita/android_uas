<?php
	...
	class Barang extends REST_Controller {
		...
		function barang_post(){
			...
			switch ($action) {
				case 'insert':
					$this->insertBarang($data_barang);
					break;
				case 'update':
					$this->updateBarang($data_barang);
					break;
				case 'delete':
					$this->deleteBarang($data_barang);
					break;
				default:
					$this->response(
							array(
		...
		function updateBarang($data_barang){
			if(empty($data_barang['nama_barang']) || empty($data_barang['warna_barang']) || empty($data_barang['kategori_barang']) || empty($data_barang['berat_barang']) || empty($data_barang['deskripsi']) || empty($data_barang['harga']) || empty($data_barang['stok'])){
				...
				if($get_barang_baseID === 0){
					...
					if($data_barang['foto']){
						$get_photo_barang =$this->db->query("
	                    SELECT foto
	                    FROM barang
	                    WHERE id_barang = {$data_barang['id_barang']}")->result();
	            
	                    if(!empty($get_photo_barang)){
		...
		function deleteBarang($data_barang){
